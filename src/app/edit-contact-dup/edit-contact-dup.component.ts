  ...
  var self=this;
      ...
      this.websocket.onmessage = function (event) {
        ...
        if (result_message[0].data[0].calltype == "Incoming Call" && result_message[0].data[0].ag_no == $('#dup_user_number').val()) {
          $('#dup_queue_ids2').val(result_message[0].data[0].q_no);
...
getAuxCatogory() {
  ...
  getAuxCode() {
    ...
    //  return false;
    let cat_id = $('#dup_auxcodes_pop').val();
...
let assigned_department_id: any= $('#dup_departments').val();
console.log(assigned_department_id);

let res_department_id: any= $('#dup_res_departments').val();
console.log(assigned_department_id);
...
return false;  
  ...
  $('#dup_auxcodes_pop').val('');
  $('#dup_auxcodes_subcat').val('');
  $('#dup_notes').val('');
}
acceptIncomeCall() {
