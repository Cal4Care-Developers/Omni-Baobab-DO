...
logoutClick(){
    let access_token: any=localStorage.getItem('access_token');
    let api_req:any = '{"operation":"call", "moduleType":"call", "api_type": "web", "access_token":"'+access_token+'", "element_data":{"action":"in_login_logout","agent_id":"'+this.uadmin_id+'","reason":"Logoff","status":"0"}}';
    ...
    this.serverService.sendServer(api_req).subscribe((response:any) => {
        ...
        var socket_message  =  '[{"cust_id":"'+this.has_hard_id+'","data":[{"Name":"omnilogoff","extension":"'+this.extension+'","status":"0","reason":"logoff"}]}]';
        this.websocket.send(socket_message);
      if(response.result.status == true){
        // iziToast.success({
